...
const AddTaskForm: React.FC<AddTaskFormProps> = ({ isOpen, onClose, fetchTasks }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 overflow-y-auto" onClick={onClose}>
      <div className="bg-[#FEFDF2] rounded-xl p-6 md:p-8 relative w-[90vw] max-w-[600px] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <button className="absolute top-4 right-4 text-black text-xl" onClick={onClose}>
          X
        </button>
        <h2 className="text-[20px] md:text-[30px]">ADD NEW TASK</h2>
        <p className="text-[#BDBDBD] mb-4 text-[15px] md:text-[20px]">Enter your task details here!</p>
